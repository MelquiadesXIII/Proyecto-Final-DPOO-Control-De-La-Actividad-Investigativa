...
public class RemoverParticipanteDialog extends JDialog{
	...
	public RemoverParticipanteDialog(final JFrame parent, CursoPosgrado curso) {

		super(parent, "Remover Participante", true);
		this.parent = parent;
		this.curso = curso;
		...
		ArrayList<Docente> elementosDisponibles = curso.getParticipantes();
		...
		JLabel lblTitulo = new JLabel("Remover Participante");
	...
	private void procesarSeleccion() {
		...
		if (selectedIndex != -1) {
			...
			MensajeDialog m1 = new MensajeDialog(parent, "¿Seguro que desea remover al docente?", Tipo.CONFIRMACION);    
			m1.setVisible(true);

			if(m1.isConfirmado()){
				try {
					curso.removerParticipante(seleccion);

					MensajeDialog d = new MensajeDialog(parent, "Docente eliminado correctamente", Tipo.RETROALIMENTACION);
					d.setVisible(true);
					confirmado = true;
					dispose();
