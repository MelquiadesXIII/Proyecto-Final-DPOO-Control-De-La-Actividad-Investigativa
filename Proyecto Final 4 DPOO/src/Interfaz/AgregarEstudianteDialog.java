...
public class AgregarEstudianteDialog extends JDialog {
    ...
    public AgregarEstudianteDialog(final JFrame parent, final Vicedecanato vicedecanato, final Departamento departamentoActual) {
        ...
        ArrayList<Estudiante> elementosDisponibles = this.vicedecanato.obtenerEstudiantesNoRegistradosEnDepartamentos();
    ...
    private void procesarSeleccion() {
        ...
        if (selectedIndex != -1) {
            Estudiante seleccion = (Estudiante) listaElementos.getModel().getElementAt(selectedIndex);

            MensajeDialog m1 = new MensajeDialog(parent, "¿Seguro que desea agregar al estudiante?", MensajeDialog.Tipo.CONFIRMACION);    
            m1.setVisible(true);

            if(m1.isConfirmado()){
                try {
                    departamentoActual.agregarEstudiante(seleccion);

                    MensajeDialog d = new MensajeDialog(parent, "Estudiante agregado correctamente", MensajeDialog.Tipo.RETROALIMENTACION);
                    d.setVisible(true);
                    confirmado = true;
                    dispose();
                } catch(RuntimeException r) {
                    MensajeDialog m2 = new MensajeDialog(parent, r.getMessage(), MensajeDialog.Tipo.RETROALIMENTACION);
                    m2.setVisible(true);
                }
            }
        } else {
            MensajeDialog d = new MensajeDialog(parent, "Seleccione un estudiante de la lista", MensajeDialog.Tipo.RETROALIMENTACION);
            d.setVisible(true);
        }
    }
