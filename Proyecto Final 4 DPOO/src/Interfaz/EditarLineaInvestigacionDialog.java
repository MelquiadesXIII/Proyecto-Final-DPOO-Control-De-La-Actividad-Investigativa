...
public class EditarLineaInvestigacionDialog extends JDialog {
	...
	public EditarLineaInvestigacionDialog(final JFrame parent, final Vicedecanato vicedecanato,final Departamento departamento,final LineaInvestigacion linea) {
		super(parent, "Editar Línea de Investigación", true);
		setUndecorated(true);
		...
		comboEncargado.addItem(linea.getResponsable());

		ArrayList<Docente> docentesValidosParaSerEncargados = departamento.obtenerListaDocentesValidosParaSerEncargadosEnNuevaLineaDeInvestigacion();

		for (Docente d : docentesValidosParaSerEncargados) 
		...
		JButton botonCrear = new JButton("Aceptar");
		...
		botonCrear.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try{
					...
					linea.setResponsable(seleccionado);
					MensajeDialog d = new MensajeDialog(parent, "La línea ha sido editada satisfactoriamente", Tipo.RETROALIMENTACION);
					d.setVisible(true);
					confirmado = true;
					dispose();
		...
		JLabel lblTitulo = new JLabel("Editar L\u00EDnea Inves.");
