...
public class BajaMaestriaDialog extends JDialog{
	...
	public BajaMaestriaDialog(final JFrame parent, final Maestria maestria, final Departamento departamentoActual) {

		super(parent, "Baja Docente", true);
		this.parent = parent;
		...
		ArrayList<Docente> elementosDisponibles = maestria.getMatriculados();
		...
		btnAgregar = new JButton("Dar de baja");
		...
		JLabel lblTitulo = new JLabel("Baja Docente");
	...
	private void procesarSeleccion() {
		...
		if (selectedIndex != -1) {
			...
			MensajeDialog m1 = new MensajeDialog(parent, "¿Seguro que desea dar de baja al docente?", Tipo.CONFIRMACION);    
			m1.setVisible(true);

			if(m1.isConfirmado()){
				try {
					maestria.removerMatriculado(seleccion);

					MensajeDialog d = new MensajeDialog(parent, "Docente dado de baja correctamente", Tipo.RETROALIMENTACION);
					d.setVisible(true);
					confirmado = true;
					dispose();
