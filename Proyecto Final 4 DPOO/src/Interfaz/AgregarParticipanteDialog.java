...
public class AgregarParticipanteDialog extends JDialog {
	...
	public AgregarParticipanteDialog(final JFrame parent, final Maestria maestria, CursoPosgrado curso) {

		super(parent, "Agregar Participante", true);
		this.parent = parent;
		this.curso = curso;
		...
		ArrayList<Docente> elementosDisponibles = maestria.obtenerListaDocentesNoParticipantesEnCurso(curso);
		...
		JLabel lblTitulo = new JLabel("Agregar Participante");
	...
	private void procesarSeleccion() {
		...
		if (selectedIndex != -1) {
			...
			if(m1.isConfirmado()){
				try {
					curso.agregarParticipante(seleccion);
					MensajeDialog d = new MensajeDialog(parent, "Participante agregado correctamente", Tipo.RETROALIMENTACION);
					d.setVisible(true);
					confirmado = true;
					dispose();
